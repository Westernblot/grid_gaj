...
(function ($) {
	...
	$.addFlex = function (t, p) {
			...
			addData: function (data) { //parse data
				...
				if (p.dataType == 'json') {
					$.each(data.rows, function (i, row) {
						...
						$('thead tr:first th', g.hDiv).each(function (index) {//add cell
								var td = document.createElement('td');
								var idx = $(this).attr('axis').substr(3);
								td.align = this.align;
								// If each row is the object itself (no 'cell' key)
								if (typeof row.cell == 'undefined') {
									if(p.colModel[idx].checkbox){  //添加checkbox --gxl      
			                            var cthch = $( "<input/>");  
			                            $(cthch).attr("type","checkbox");
			                            $(cthch).attr("name",p.colModel[idx].name);
			                            $(cthch).attr("value",row[p.colModel[idx].name]);
			...
			addRowProp: function () {
				$('tbody tr', g.bDiv).on('click', function (e) {
					...
					if (p.singleSelect && ! g.multisel) {
						$(this).siblings().removeClass('trSelected').find("input[type='checkbox']").removeAttr("checked");                                      
					}
		...
		if (p.colModel) { //create model if any
			...
			for (var i = 0; i < p.colModel.length; i++) {
				...
				if( cm ) {	// only use cm if its defined
					...
					if( cm.checkbox){//--gxl表头添加checkbox，可以全选或者取消
						$(th).attr('checkbox', "true");
						var cthch = $('<input type="checkbox" name="checkAll"/>');  
	                    $(th).append(cthch);
	           			$(th).on('click',"input[type=checkbox]",function(){ 
                        if($(this).attr("checked")){  
                        	 $('tbody tr',g.bDiv).each(function(){  
	                                $(this).addClass('trSelected').find('input[type="checkbox"]')[0].checked=true;  
	                            });  
                        }else{  
                        	$('tbody tr',g.bDiv).each(function(){  
                                $(this).removeClass('trSelected').find('input[type="checkbox"]')[0].checked=false;  
                            });  
                        }  
                     });  
					}
					...
					if (cm.hide) {//课自由选择隐藏或显示当列
						th.hidden = true;
					}
					if (cm.show!=undefined && !cm.show) {//是否完全不可见
						$(th).attr('show',"false");
						th.hidden = true;
					}
					if (cm.process) {
		...
		if ($('th', g.hDiv).length) {
				...
				var cn = 0;
				$('th div', g.hDiv).each(function () {//显示那些标题可以显示--gxl
					...
					//checkbox 不可隐藏
					if ( $(kcol).attr("checkbox") != 'true' && $(kcol).attr("show")!= "false") {
