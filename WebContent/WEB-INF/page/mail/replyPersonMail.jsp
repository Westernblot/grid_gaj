...
function sendMail(){
	...
	if(data>0){
		alert('发送成功!');
		location.href="mSendPersonMail.do";
	}
}
...
<div id="div_right">
	<div class="right_top">
		...
		<form action=""  id="form1">
		   <input type="hidden" name="fId" value="${pData.F_ID }">
		   <table width="100%" border="0" class="mtable" >
		   <tr>
		   <td  width="50px">收件人</td>
		   <td  >
		   <input type="hidden" size="80" id="fReceivedId" name="fReceivedId" value="${pData.F_SEND_ID }" want="yes" title="收件人" readOnly >
		   
		   <input type="text" size="80" id="fReceived" name="fReceived" value="${pData.F_SEND }" readOnly>
	  ...
	  <span id="pageNo"></span>/<span id="totalPage"></span>页,共 <span id="totalRecord"></span>条,每页 20 条	    
		   ...
		   <td >主题</td>
		   <td >
		   <input type="text" size="100" id="fSubject" name="fSubject" value="回复：${pData.F_SUBJECT }" >
		   </td>
		   </tr>
		   ...
		   <input type="checkbox" id="fSms" name="fSms" value="是" 
		     onclick="showTrMsg()" ><label for="fSms">是否短信提醒</label>
		       &nbsp;&nbsp;&nbsp;&nbsp;<b> 重复提醒时间：</b>
		  ...
		  如不填，则默认为：<font color="red">您有一封主题为《XXXX》的邮件，请注意查收!</font>
		   ...
		   <td >内容</td>
		   <td >
		   <textarea id="fContent" name="fContent" style="width: 100%; height:300px;visibility: hidden;">
		    ...
		    --------------原始邮件-------------
		    <br>
		    <b>发件人：</b>${pData.F_SEND }<br>
		    <b>发件时间：</b>${pData.F_SEND_TIME }<br>
		    <b>收件人：</b>${pData.F_RECEIVED }<br>
		    <b>主题：</b>${pData.F_SUBJECT }<br>
		    ${pData.F_CONTENT}<br>
		    </textarea>
		   <br>
