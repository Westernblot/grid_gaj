...
function doSave(){
	...
	if(res>0){
		alert('操作成功!');
		location.href="mRedNewsMain.do?fGroup="+fGroup;
	}
}
...
function doUpload(){
    ...
    (
        {
            url: 'sUpload/redNewsAttachment', //用于文件上传的服务器端请求地址
            type: 'post',
...
<div id="div_right">
	<div class="right_top">
		...
		<form action="${news==null?'mNewsInsert.do':'mNewsUpdate.do'}"  id="form1">
		   ...
		   是否红头文件：
		   <select id="fRed" name="fRed">
		   
		   <option value="非红头样式"  <c:if test="${news.fRed=='非红头样式' }">selected</c:if>   >非红头样式</option>
		   <option value="黄石治安简报"  <c:if test="${news.fRed=='黄石治安简报' }">selected</c:if>   >黄石治安简报</option>
		   <option value="黄石市公安局"  <c:if test="${news.fRed=='黄石市公安局' }">selected</c:if>   >黄石市公安局</option>
		   <option value="黄石市公安局政治部文件"  <c:if test="${news.fRed=='黄石市公安局政治部文件' }">selected</c:if>   >黄石市公安局政治部文件</option>
		   </select>
		   <font color="red">*</font>
		   </td>
		   </tr>
		   ...
		   <tr>
		   <td align="right" width="15%">来源/字号：</td>
		   <td colspan="3" >
		   <input type="text" size="80" id="fSource" name="fSource" value="${news.fSource}">
		   ...
		   <td colspan="3">
		   <textarea rows="3" cols="30" id="fCopy" name="fCopy">${news.fCopy }</textarea>
		   </td>
		   </tr>
		   ...
		   <td></td>
		   <td colspan="3">
		   签发：<input type="text" id="fSign" name="fSign" value="${news.fSign }">
		  核稿：<input type="text" id="fCheckDraft" name="fCheckDraft" value="${news.fCheckDraft }">
		  承办：<input type="text" id="fUndertaker" name="fUndertaker" value="${news.fUndertaker }">
		   </td>
		   </tr>
		   ...
		   <c:if test="${fn:contains(news.fRecDept,dept.fName)}">checked</c:if>
		   >
		   <label for="chk_${vs.index}">${dept.fName}</label>
		   
		   <c:if test="${(vs.index+1)%6==0 }"><br></c:if>
