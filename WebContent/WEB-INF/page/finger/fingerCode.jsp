...
function doSearch(){
	...
	$('#formQuery').attr('action','mTsFingerCode.do');
	$('#formQuery').submit();
}
...
function doExcel(){
	...
	$('#formQuery').attr('action','fingerExcelByCode2Month.do');
	$('#formQuery').submit();
}
	...
	<div class="right_top">
				...
				<%--
				 ...
				 --%>
					...
					<form action="mTsFingerCode.do"   id="formQuery" method="post">
					  月份: <input type="text" id="fTime" name="fTime" value="${(finger.fTime==null or finger.fTime=='')?nowMonth:(finger.fTime)}" class="Wdate"  onclick="WdatePicker({dateFmt:'yyyy-MM'})">
					  警号：<input type="text" id="fCode" name="fCode" value="${finger.fCode==-1?'':finger.fCode }">
