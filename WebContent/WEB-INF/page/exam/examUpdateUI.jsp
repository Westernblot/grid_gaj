...
<div id="div_right">
	<div class="right_top">
		...
		<div class="develop">
		<form action="mExamUpdate.do"  id="form1" >
		   <input type="hidden" id="fId" name="fId" value="${exam.fId }">
		   <table width="100%" border="0" class="mtable" >
		   <caption>
		      <h2>${exam.fYear }年全市治安系统重点工作考核</h2>
		   </caption>
		   <tr>
		   <td colspan="6">
		   单位：<select name="fName" id="fName">
		   <c:forEach items="${deptList}" var="dept">
		   <option value="${dept.fName }" <c:if test="${exam.fName==dept.fName }">selected</c:if> >${dept.fName }</option>
		   </c:forEach>
		   
		   </select>
		   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		   考核月份：<input type="text" id="fMonth" name="fMonth" value="${exam.fMonth }" class="Wdate"  onclick="WdatePicker({dateFmt:'yyyy-MM'})" >
		         至     <input type="text" id="fEndMonth" name="fEndMonth" value="${exam.fEndMonth }" class="Wdate"  onclick="WdatePicker({dateFmt:'yyyy-MM'})" >
		   <font color="red">*</font>
		   &nbsp;&nbsp;
		  <font color="red">提示：加分和减分 请勿输入负数</font>
		   ...
		   <td width="70%"><b>项目<font color="red">*</font></b>
		   </td>
		   ...
		   <td width="20%"><b>说明</b></td>
		     ...
		     <tr>
		      ...
		      <td><select id="fItemId" name="fItemId">
		      <c:forEach items="${itemList }" var="item"> 
		      <option value="${item.fId }" <c:if test="${exam.fItemId==item.fId }">selected</c:if>  >${item.fItem }</option>
		      </c:forEach>
		      </select></td>
		      
		      <td>
		      <input type="text" id="fAdd" name="fAdd" value="${exam.fAdd }" >
		      </td>
		      <td>
		      <input type="text" id="fSub" name="fSub" value="${exam.fSub }" >
		      </td>
		      <td>
		      <input type="text" id="fRemark" name="fRemark" value="${exam.fRemark }">
		      </td>
