...
//给部门责任人赋值
function giveDutyPerson(obj){
	var fDept=$(obj).val();
	var time=new Date().getTime();
	$.ajax({
		url:'mFindDutyPerson.do?time='+time,
		data:{'fDept':fDept},
		dataType:'text',
		success:function(data){
			//alert(data);
			$('#fDutyPerson').attr('value',data);
		},
		error:function(){
			alert("访问失败!");
		}
...
<div id="div_right">
	<div class="right_top">
		...
		<form action="${task==null?'mTaskInsert.do':'mTaskUpdate.do' }"  id="form1" >
		   ...
		   <caption>
		      <h2>   
		   <select name="fDept" id="fDept" want="yes" title="部门" onchange="giveDutyPerson(this)" >
		   <option value="" >-请选择-</option>
		   <c:forEach items="${deptList}" var="dept">
		   <option value="${dept.fName }" <c:if test="${task.fDept==dept.fName }">selected</c:if> >${dept.fName }</option>
		   </c:forEach>
		   </select>	
		   重点工作安排</h2>
		   </caption>
		   ...
		   <textarea rows="3" cols="30" id="fWork" name="fWork">${task.fWork }</textarea>
		     ...
		     <textarea rows="3" cols="20" style="width: 700px" id="fFitDept" name="fFitDept" >${task.fFitDept }</textarea>
		      ...
		      <td>责任人：</td>
		      <td colspan="4">
		     <input type="text" size="100" id="fDutyPerson" name="fDutyPerson" value="${task.fDutyPerson }" >
		     <br>
