...
<div id="div_right">
	<div class="right_top">
				...
				<%--
				 ...
				 --%>
					<a href="javascript:doAudit('一级审核通过')" class="btn btn-green">批量审核通过</a> 
					<a href="javascript:doAudit('审核未通过')" class="btn btn-red">批量审核不通过</a> 
					
					<form action="mAuditMain1.do" style="display:none"  id="formQuery" method="post">
		...
		<div class="develop">
				...
				<c:when test="${not empty newsList}">
						 <c:forEach items="${newsList}" var="news">
						  ...
						  <td>${news.fSort }</td>
						  <td>${news.fGroup }</td>
						  <td><a href="auditUI1.do?fId=${news.fId}">审核</a></td>
						  </tr>
						 </c:forEach>
				</c:when>
				<c:otherwise>
